...
class MainActivity : ComponentActivity() {
    ...
    private fun MainScreen() {
        ...
        val permissionLauncher = rememberLauncherForActivityResult(
            ...
            status = if (granted.containsAll(HealthConnectReader.requiredPermissions)) {
        ...
        Column(
            ...
            Text("Android 16 build. Reads today's steps and publishes them to Home Assistant via MQTT.")
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                Button(onClick = {
                    ...
                    scope.launch {
                        ...
                        if (result.steps != null) lastSteps = result.steps
                    }
                }) {
                    Text("Sync now")
                }
            ...
            Text("Last steps: ${lastSteps?.toString() ?: "not synced yet"}")
            Text("Status: $status")
        }
    }

    data class SyncResult(val message: String, val steps: Long? = null)
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (!reader.hasAllPermissions()) {
                    return@withContext SyncResult("Grant Health Connect steps permission first")
                }
                val steps = reader.readStepsToday()
                MqttPublisher(settings).publishSteps(steps)
                SyncResult("Published steps_today=$steps", steps)
            } catch (e: Throwable) {
